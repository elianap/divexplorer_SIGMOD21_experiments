...
def import_process_heart(discretize=False, bins=3, inputDir=DATASET_DIR):
    ...
    check_dataset_availability("heart.csv", inputDir=inputDir)
...
def import_process_compas(discretize=False, risk_class=False, inputDir=DATASET_DIR):
    ...
    check_dataset_availability("compas-scores-two-years.csv", inputDir=inputDir)
...
def import_process_bank(bins=3, discretize=False, inputDir=DATASET_DIR):
    check_dataset_availability("datasets_4471_6849_bank.csv", inputDir=inputDir)
...
def train_predict_old(
    ...
    if type_cl == "RF":
        ...
        toScale = True
        clf = make_pipeline(
            StandardScaler(), SVC(random_state=42, probability=True, **args)
        )
    elif type_cl == "XGB":
        import xgboost as xgb

        clf = xgb.XGBClassifier(random_state=42, **args)
    else:
        from sklearn.ensemble import RandomForestClassifier
    ...
    if validation != "cv":
        ...
        if fold == "stratified":
            ...
            cv = StratifiedKFold(n_splits=k_cv, random_state=42)
        else:
            from sklearn.model_selection import KFold

            cv = KFold(n_splits=k_cv, random_state=42)
        y_predicted = cross_val_predict(
            clf, X, y, cv=cv
        )  # if toScale==False else cross_val_predict(pipeline, X, y, cv=cv)
        X_FP, y_FP = X, y
